...
const web = {
    ...
    resolveEditor: () =>[{ section: 'editor', json: JSON.stringify(pages, null, 4) }],
    updateDataset: function() {
        try {
            const newPages = JSON.parse(web.gebi('json-input').value);
            const el = web.gebi('preview-area');
            el.innerHTML = newPages.home.map(d => components[d.section]?.(d) || '').join('');
        } catch(e) { alert("JSON Error: " + e.message); }
    },
    downloadDataset: function() {
        const data = web.gebi('json-input').value;
    ...
    // Tampilkan di editor dalam format pages.key = [...]
    resolveEditor: () => [{
        section: 'editor',
        json: pagesToFileFormat(pages)
    }],
    ...
    try {
        ...
        // Buat sandbox pages sementara, eval isi editor
        const tempPages = {};
        ...
        const activeKey = web.currentPage || 'home';
        const el = web.gebi('preview-area');
        el.innerHTML = (pages[activeKey] || [])
...
// Download semua halaman dalam satu file dataset.js
downloadDataset: function() {
...
// Download per halaman (opsional)
downloadPage: function(key) {
...
// Load halaman tertentu ke editor
...
// resolveEditor — default load halaman pertama dari manifest
resolveEditor: () => {
    const key = pageFiles[0];
    return [{
        section: 'editor',
...
// Update — eval sandbox, update pages global, preview halaman aktif di select
updateDataset: function() {
    try {
        const input = web.gebi('json-input').value;
        const tempPages = {};
        ...
        const key = web.gebi('page-select')?.value || pageFiles[0];
        const el = web.gebi('preview-area');
...
// Download semua halaman → dataset.js
downloadDataset: function() {
    const data = pagesToFileFormat(pages);
    const blob = new Blob([data], { type: 'application/javascript' });
    const a = document.createElement('a');
...
// Download halaman yang sedang aktif di select → key.js
...
editor: (d) => `
    <div class="row page">
        <div class="row">
            <h3>JSON Dataset Editor</h3>
            <textarea id="json-input" style="width:100%; height:400px; font-family:monospace; padding:10px;">${d.json}</textarea>
            ...
            <button class="slcBtn" style="background:#555;" onclick="web.downloadPage()">Download page</button>
        </div>
        <div class="row">
            <h3>Preview</h3>
    ...
    <div class="row page">
        <div class="row">
            ...
            <div style="margin-bottom:10px; display:flex; gap:8px; align-items:center;">
                <label>Halaman:</label>
                <select id="page-select" onchange="web.loadPageToEditor(this.value)" style="padding:6px 10px; font-size:14px;">
                    ${pageFiles.map(key => `<option value="${key}">${key.charAt(0).toUpperCase() + key.slice(1)}</option>`).join('')}
                </select>
            </div>
            <textarea id="json-input" style="width:100%; height:400px; font-family:monospace; padding:10px;">${d.json}</textarea>
            <button class="slcBtn" onclick="web.updateDataset()">Update Preview</button>
